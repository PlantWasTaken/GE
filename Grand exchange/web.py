...
table = PrettyTable()
table.field_names = ['Name', 'Item id', 'Price', 'Change', 'Trend']
...
def fprint(p_change, data): #called on rep
    #print(str(data['item']['name']) + " : " + str(data['item']['current']['price']) + " | " + str('Change') + " : " + str(round(p_change, 2)) + " | " + str('Trend') + " : " + str(data['item']['today']['trend']) + "\n")
    #prices.write(str(data['item']['name']) + " : " + str(data['item']['current']['price']) + " | " + str('Change') + " : " + str(round(p_change, 2)) + " | " + str('Trend') + " : " + str(data['item']['today']['trend']) + "\n")
    table.add_row([data['item']['name'], data['item']['id'], data['item']['current']['price'], str(round(p_change, 2)), data['item']['today']['trend']])
    eval(data, p_change) #eval of item | looks for pricefall between 3-8%
...
def findItemPrice(): #finds all item prices from index.txt
    Done = False
    i = 0
    with alive_bar(line_count, title='Finding prices', length=20) as bar: #progress bar decleration
        while(Done == False):
            while(i != line_count):
